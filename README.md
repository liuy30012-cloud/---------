...
- ✅ **后端配置安全加固** — 敏感配置项集中管理与保护
  - Elasticsearch 密钥配置从明文迁移到环境变量
  - 水印密钥、反爬虫配置统一使用环境变量管理
  - 测试环境配置文件添加安全占位符
- ✅ **借阅状态常量集中化** — 重构 BorrowRecord 状态管理
  - 新增 `STATUS_BORROWED`、`STATUS_RETURNED`、`STATUS_OVERDUE` 等常量
  - 统一 BorrowService、ReservationService、测试代码中的状态字符串
  - 消除硬编码字符串，提升代码可维护性

#### 🔧 代码优化

- ✅ **鉴权错误处理完善** — 优化 JWT 认证失败响应
  - 统一错误响应格式，提供清晰的错误信息
  - 改进登录页面交互，简化用户体验
- ✅ **BookController 代码清理** — 移除冗余导入和未使用代码
- ✅ **测试代码优化** — BorrowIntegrationTest 和 BorrowServiceTest 使用状态常量
